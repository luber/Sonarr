...
//                          RegexOptions.IgnoreCase | RegexOptions.Compiled),
		...
		public static ParsedEpisodeInfo CustomParseTitle(string title, Language defaultLanguage, Regex[] regexes)
		{
			try
			{
				if (!ValidateBeforeParsing(title)) return null;
				...
				foreach (var regex in regexes)
				{
					...
					if (match.Count != 0)
					{
						...
						try
						{
							...
							result.SeriesTitleInfo.Year = ParseNumber(match[0].Groups["year"].Value);
							...
							var fakeTitleForQuality = string.Empty;
							var quality = match[0].Groups["quality"].Value;
							if (!string.IsNullOrWhiteSpace(quality))
							{
								fakeTitleForQuality = quality + "p";
							}

							var webdl = match[0].Groups["webdl"].Value;
							if (!string.IsNullOrWhiteSpace(webdl))
							{
								if (webdl.Trim().ToLower().EndsWith("rip")) //e.g. HDTVRip
								{
									webdl = webdl.Substring(0, webdl.LastIndexOf("Rip", StringComparison.CurrentCultureIgnoreCase));
									if (string.Equals(webdl, "WEB", StringComparison.InvariantCultureIgnoreCase))
									{
										webdl = "WEB-DL";
									}
								}
								fakeTitleForQuality += " " + webdl;
							}

							if (!string.IsNullOrEmpty(fakeTitleForQuality))
							{
								result.Quality = QualityParser.ParseQuality(fakeTitleForQuality);
							}
							...
							result.ReleaseGroup = match[0].Groups["relgrp"].Value;
							Logger.Debug("Release Group parsed: {0}", result.ReleaseGroup);
							...
							//		result.ReleaseGroup = subGroup;
							//	}
							//if (string.Equals(result.ReleaseGroup, "LostFilm", StringComparison.CurrentCultureIgnoreCase) ||
						 //           string.Equals(result.ReleaseGroup, "ColdFilm", StringComparison.CurrentCultureIgnoreCase))
							//{
							//	result.Language = Language.Russian;
							//}
